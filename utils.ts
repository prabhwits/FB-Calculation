...
export function mapItemToTreeNode(items: any): TreeNode[] {
  ...
  const baseItems = items.filter((item: any) => {
    ...
    return item.tags.some((tag: any) => {
      ...
      return tag.list.some(
        (sub: any) => sub.code === "type" && sub.value === "item"
      );
    });
  });
    ...
    if (item.price && Array.isArray(item.price.tags)) {
      for (const tag of item.price.tags) {
        if (tag.code === "default_selection") {
          ...
          const maxVal = tag.list.find(
            (x: any) => x.code === "maximum_value"
          )?.value;
          if (minVal && maxVal) {
            default_selection = {
...
export function mapCustomItemToTreeNode(items: any[]): TreeNode[] {
  ...
  const customItems = items.filter((item) => {
    ...
    return item.tags.some((tag: any) => {
      if (tag.code !== "type" || !Array.isArray(tag.list)) return false;
      return tag.list.some(
        (sub: any) => sub.code === "type" && sub.value === "customization"
      );
    });
  });
    ...
    if (Array.isArray(item.tags)) {
      for (const tag of item.tags) {
        if (tag.code === "custom_group" && Array.isArray(tag.list)) {
          ...
          const parentEntry = tag.list.find(
            (entry: any) => entry.code === "id"
          );
          if (parentEntry) parentId = parentEntry.value;

          // Check if the customization is default
          const defaultEntry = tag.list.find(
            (entry: any) => entry.code === "default" && entry.value === "yes"
          );
...
export function mapCustomizationsToBaseItems(
  baseItems: TreeNode[],
  customItems: TreeNode[]
): TreeNode[] {
